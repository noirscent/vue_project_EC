    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <!-- skuImageList是对象数组，传递给子组件的数据至少是一个对象数组 -->
...
export default {
  ...
  methods: {
    ...
    changeSkuNum(event) {
      ...
      // isNaN(value)过滤掉其他字符(包含e)，但不能过滤掉 负数、'0'、空字符串、小数
      // value < 1过滤掉负数、'0'、空字符串
    ...
    addshopcar() {
      ...
      this.$store.dispatch("addOrUpdateShopCart", {
        ...
        skuNum: String(this.skuNum),
      }).then(() => {
        // 添加成功进行路由跳转，并传递商品信息参数，但直接传递skuInfo，地址栏会有点难看
        // this.$router.push({name: 'addcartsuccess', query: {skuInfo: this.skuInfo, skuNum: this.skuNum}})
        // 将skuInfo存入本地存储中, 就无需出现在地址栏中了
        sessionStorage.setItem('skuInfo', JSON.stringify(this.skuInfo))

        // 在跳转前请求ShopCart组件需要的购物车数据, 避免渲染失败
